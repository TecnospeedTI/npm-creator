...
module.exports = function (options, callback) {
  ...
  async.parallel([
    function updateBin (done) {
      var binPath = path.join(basePath, 'bin', 'bin')
      var newBinPath = path.join(basePath, 'bin', options.name)
      fs.readFile(binPath, function (err, content) {
        if (err) return done(err)
        content = content.toString().replace(/\{NAME\}/g, options.name)
        async.series([
          function (next) { fs.writeFile(binPath, content, next) },
          function (next) { fs.rename(binPath, newBinPath, next) }
        ], done)
      })
    },
    ...
    if (err) {
      ...
      return callback(new Error('CREATING_STRUCTURE'))
    }
    callback()
  })
}
